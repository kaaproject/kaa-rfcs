...
- **Client**: an application that uses a single "connection" to server to represent one or multiple endpoints.
...
KP can not make assumptions about payload format, thus it cannot define the format for passing Request IDs within message payload.
...
In that case, it is RECOMMENDED to add payload format specifier.
...
After the client authentication is done, client may issue commands on behalf of endpoints.
To do so, client needs an [endpoint token](#language).
...
There are two possibilities for client to get an endpoint token: it is either pre-provisioned into the client-side application or firmware, or requested at run-time via an extension *(endpoint registration, endpoint authentication)*.
